                        ...
                        <div class="form-group" v-for="(email, i) in form.emails">
                            <div class="input-group">
                                <input
                                    ...
                                    :id="'email' + i"
                                    class="form-control"
                                    :class="{'is-invalid': errors.has('email'  + i, 'paymentListEmail')}"
                                    v-validate
                                    data-vv-rules="required|email"
                                    data-vv-scope="paymentListEmail"
                                ...
                                <span class="invalid-feedback" role="alert" v-if="errors.firstByRule('email' + i, 'required', 'paymentListEmail')">
                                    <strong>{{ t('validation.required', {attribute: 'email'}) }}</strong>
                                </span>
                                <span class="invalid-feedback" role="alert" v-if="errors.firstByRule('email' + i, 'email', 'paymentListEmail')">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            validateForm() {
                this.$validator.validateAll('paymentListEmail').then(res => res && this.sendForm())
            },
            ...
            sendForm() {
                ...
                ApiService.post('/payment/' + this.editData.uuid + '/email/' + lang, this.form).then(res => {
                    if (res.data.success) {
                        location.reload();
                    }
